...
**Connection screen.** A compact centred column without a card frame. The bag and pot illustration sits above the magenta sign "Готуємо з Сільпо", followed by the lime "Підключити Сільпо" button and one muted 14px phone-login note. Keep busy feedback on the button and persistent connection errors below the group.
...
| Сільпо connection     | `bag-pot.webp`       | 128px, centred above the short sign heading. Grocery bag beside an enamel pot. Keep the connection button and phone-login note visible. Do not add a card frame or technical storage copy.                                                                                      |
