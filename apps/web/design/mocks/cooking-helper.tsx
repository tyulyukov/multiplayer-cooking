  ...
  steps: [
    {
      ...
      body: "Помий помідори та наріж їх кубиками.",
      kind: "task" as const,
      slots: [1],
      dependsOn: [],
      activeMinutes: 3,
    ...
    {
      id: "sauce",
      title: "Приготувати томатний соус",
      body: "Прогрій оливкову олію. Додай томати, часник і чилі, готуй 5 хвилин на середньому вогні.",
      kind: "task" as const,
      slots: [1],
      dependsOn: ["prep"],
      activeMinutes: 5,
      equipment: [],
      checklist: [{ id: "oil", label: "Прогріти олію" }],
      timers: [],
    },
    {
      id: "pasta",
      title: "Зварити пасту",
      body: "Відвари пасту в підсоленій воді. Залиш трохи води для соусу.",
      kind: "task" as const,
      slots: [2],
      dependsOn: [],
      activeMinutes: 2,
...
const nextPlan = {
  ...
  steps: plan.steps.map((step) =>
    step.id === "sauce"
      ? {
          ...step,
          body: "Прогрій оливкову олію. Додай томати й часник, готуй 5 хвилин на середньому вогні. Перець чилі не додавай.",
        }
      : step,
  ),
};
...
const initialData: CookingRoomData = {
  ...
  members: [
    me,
    { ...me, _id: "member-guest" as Id<"cookingMembers">, name: "Аня", role: "cook", slots: [2] },
  ],
  steps: plan.steps.map((step) => ({
    stepKey: step.id,
    status: step.id === "prep" ? "done" : "active",
    slots: step.slots,
    readyMemberIds: [],
    checkedIds: [],
    startedAt: now - 10000,
...
const initialMessages: Props["messages"] = [
  {
    ...
    role: "user",
    authorName: "Оля",
    text: "Не хочу додавати чилі. Можна без нього?",
    createdAt: now - 2000,
  },
  {
    _id: "answer",
    role: "assistant",
    text: "Так, готуй без чилі. Соус буде м’якшим, час приготування той самий.\n\nЯкщо перець ще не додала, просто пропусти його.",
    createdAt: now - 1000,
  },
];
const initialProposal: Props["proposals"][number] = {
  ...
  status: "open",
  planVersion: 1,
  preview: "Приберемо чилі з інгредієнтів і з кроку приготування соусу.",
  plan: nextPlan,
  affectedStepKeys: ["sauce"],
  createdAt: now - 500,
};
...
export const Fixture: FC = () => {
    ...
    mode === "empty" || mode === "closed"
      ...
      : mode === "long"
        ? Array.from({ length: 20 }, (_, index) => ({
            ...initialMessages[index % 2]!,
            _id: `message-${index}`,
            createdAt: now - 30000 + index * 1000,
          }))
        : initialMessages,
  );
    ...
    complete: (key) =>
      setData((current) => ({
        ...
        steps: current.steps.map((step) =>
          step.stepKey === key ? { ...step, status: "done" } : step,
        ),
      })),
    undo: () => {},
  ...
  return (
    <>
      ...
      <CookingHelper
        chatRequestKey={chatRequestKey}
        plan={data.room.plan!}
        contextStepKey={stepKey}
        onContextChange={setStepKey}
        messages={messages}
        proposals={proposals}
        notes={notes}
        loaded
        helperBusy={busy}
        helperError={mode === "error" ? "Fixture failure" : undefined}
        open={open}
        ...
        onAsk={async (text) => {
          ...
          setMessages((current) => [
            ...current,
            {
              ...
              text,
              createdAt: Date.now(),
              attachmentUrls: attachments.items.map(() => "/images/cooking-prep-photo.webp"),
            },
          ]);
          setBusy(true);
          window.setTimeout(() => {
            setMessages((current) => [
              ...current,
              {
                _id: crypto.randomUUID(),
                role: "assistant",
                text: "Так, можна без чилі. Продовжуй готувати соус із томатами та часником.",
                createdAt: Date.now(),
              },
            ]);
            setBusy(false);
        ...
        onApprove={async (id) => {
          setData((current) => ({ ...current, room: { ...current.room, plan: nextPlan } }));
        ...
        onNote={async (text) => {
          setNotes((current) => [
            ...current,
            {
              _id: crypto.randomUUID() as Id<"cookingNotes">,
              authorMemberId: me._id,
              text,
              createdAt: Date.now(),
              canDelete: true,
