...
The values in this file are the source of truth. `../../packages/theme/src/index.ts` and `../../packages/theme/src/web.css` must match them. When they differ, fix the theme package.
...
**Connection screen.** The approved [morning kitchen concept](design/mocks/starter-kitchen-reference.png) replaces the small illustration column. A full-width cream wall, teal tile backsplash, and counter frame the central login controls. This screen uses a 28px checker band instead of the shared 48px band to match the selected composition. The heading-to-button gap is 36px; the other content gaps are 24px. The grocery bag stays on the left and the pot on the right. Use `starter-kitchen.webp` from 1200px, `starter-kitchen-tablet.webp` from 640px, and `starter-kitchen-mobile.webp` below it. Cover the page without stretching and align the image to the bottom. Desktop uses `image-set()` to choose the 1920 × 1200 standard export or the 3840 × 2400 Retina export. Inspect background detail at actual 2× device density, including the enlargement caused by `cover`; a 1× screenshot is not sufficient for full-screen artwork. The mobile composition keeps both objects visible. A separate transparent `starter-sign.webp` hangs above the live heading "Що приготуємо сьогодні?". The lime connection button, phone-login note, busy state, and persistent errors remain real UI. The sign is decorative, and image failures leave usable text and controls. Keep the scene static and use a minimum page height with normal scrolling on short screens.

**Profile menu.** An ink tag in the top right with the user icon and the person's name, with "Профіль" as the fallback, opening a shadcn dropdown: 2px ink border, radius 14px, items in Golos 600 15px with teal icons. Under the name, "Телефон:" and "Пошта:" rows in Golos 13px show the value blurred as scrambled characters of the same length, so the row does not change width when a tap reveals it. Phone numbers stay on one line, and long email addresses wrap within the menu. "Вийти" is the destructive item and sits last after a separator.
...
**Dialog titles.** Every Dialog, Sheet, and Drawer opens with one Oswald 600 title that names what the person can do there ("Історія розмов", "Скільки вас готує?", "Інструменти", "Кухарі") and no description line under it. Use 24px in account dialogs and 28px in cooking drawers. If the title needs a second sentence to be understood, the title is wrong.

**History panel.** A 560px shadcn Sheet from the right on desktop and a Drawer from the bottom on mobile, opened by the "Історія" ink tag. Each row has up to three overlapping 40px photos with 10px radius, the title in Oswald 600 17px on up to two lines or "Без назви", the time in Golos 13px `--muted-foreground`, and a ghost delete button. Time is relative and refreshes each minute: "щойно", "5 хвилин тому", "3 години тому", "вчора, 14:30", then the full localized date with time. Centre the photo stack within its fixed-width slot. The whole row is one surface: `--muted` on hover and press, 25% teal for the open thread. Keep one row per conversation, including cooked dishes. The row opens the idea conversation; a kitchen icon opens its cooking session, or a dated session menu when there are several. Link sessions by their source idea and conversation IDs. A session whose source idea was deleted remains accessible as a normal row. Only the list scrolls.
...
**Cooking setup.** "Готуємо разом" opens a shadcn Dialog on desktop and a Drawer on mobile titled "Скільки вас готує?". A stepper with two ink icon buttons sets the cook count in Oswald 700 28px. The same form collects the host name and optional equipment or experience constraints. Servings come from the recipe. The name defaults to the last successful setup in this browser, then the verified Сільпо profile name when available. "Cook again" keeps the previous session name. The lime "Створити кухню" button creates the shared session.

**Cooking lobby.** The selected [compact lobby reference](design/mocks/lobby-touch-option-2.png) places the heading and joined count on the left, with the existing oven mitts on the right at 88–112px wide. The brand, recipe sign, and lobby share a 480px maximum column with 16px mobile gutters. In the lobby, the checker band is 28px tall plus the top safe area, the brand is 17px, and the recipe title stays at 24px with wrapping for long names. A single list shows joined cooks and empty places. Tapping a joined cook opens participant management. The header has no second players button. The invite action and start button follow the list, which scrolls with the page for up to 12 cooks. Plan generation starts in the background when the room is created. The lobby shows generation errors with retry, but no routine generation status. The host can start after all requested cooks have joined, even before the plan is ready. Starting opens a shared loading screen when needed, then the timeline appears automatically. The [actual-component fixture](design/mocks/cooking-lobby.html) covers waiting, full, guest, solo, twelve-cook, long-title, loading, and error states.

**Cooking timeline.** The session has one vertical recipe timeline. The current assigned step opens by default and keeps its instruction, checklist, confirmation, timer controls, reference image, and next action together. Completed steps stay in the sequence. Upcoming steps show their owner or dependency. A compact timer dock remains available while a cook reads another step. Do not add a second dashboard for personal work.

**Cooking completion.** A finished room opens the selected [diner ticket](design/mocks/cooking-completion/b-diner-ticket.png) as a separate view. The white ticket has a 2px ink outline, a scalloped bottom border, and a chrome-framed "Смачного!" sign. The saved idea photo fills a 16:10 frame above the dish name and cook/serving counts. Keep sourced-photo attribution. Missing photos leave no empty frame; failed loads show a compact inline notice. "Приготувати ще раз" uses the existing setup flow. "До інструкцій" opens the completed timeline without changing room state, and "До завершення" returns to the ticket. Move keyboard focus to the view heading. The decorative `serving-bell.webp` sits outside the ticket at 88px on mobile and 128px on larger screens; its Blender scene and render recipe live in `design/assets/serving-bell/`.

**Cooking drawers.** "Інструменти" opens the mobile drawer for ingredients, a manual timer, unit conversion, servings, helper access, sound, and people management. People management uses its own mobile drawer. Tools do not change the timeline's order.

**Cooking helper.** A persistent bottom-right button opens a 420px chat window on desktop. The phone chat uses the available viewport height and keeps its composer above the keyboard. The conversation scrolls between the current-step selector and the fixed reply status and photo composer. "Є питання?" opens this chat with that step selected. Notes have a separate tab. Recipe changes show a short explanation, optional cooking details, and "Застосувати зміни". Applied, declined, and outdated changes remain in the conversation. The launcher moves above the timer dock when timers are visible.
...
The original six companion assets live in `public/images/`. The original companion assets are 384px square WebPs with alpha, displayed at the sizes below on mobile and desktop. Use `KitchenIllustration` for decorative companions. It supplies empty alt text, fixed dimensions, and removes a failed image without removing adjacent content. Keep illustrations static and outside the tab order.

| Placement             | Asset                                                                                                     | Display size and rule                                                                                                                                                                                                                                                           |
| --------------------- | --------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Empty history         | `recipe-box.webp`                                                                                         | 112px above the existing message. Centre the illustration and message together horizontally and vertically in the available panel space. Teal recipe box with cream index cards. Hide it while loading and when history has entries.                                            |
| Empty memories        | `memory-fridge.webp`                                                                                      | 112px above the existing message. Centre the illustration and message together horizontally and vertically in the available panel space. Tiny teal fridge door with a blank cream note and magnet. Hide it while loading and when memories exist.                               |
| Сільпо connection     | `starter-kitchen.webp`, `starter-kitchen-tablet.webp`, `starter-kitchen-mobile.webp`, `starter-sign.webp` | Full-width kitchen scene with a separate 132px mobile or 160px desktop hanging sign. Keep the centre clear for the live heading, button, note, and errors. The selected sign includes its Сільпо lettering; other illustration assets remain text-free.                         |
...
Cooking status artwork remains part of the soft 3D kitchen family. `cooking-preparing.webp` is the live plan-preparation notebook. The waiting pot `cooking-waiting.webp` is retained in the approved design prototype. Status images are decorative, use empty alt text, and the live component removes a failed image. The source PNGs live in `design/assets/`.

Recipe references are different. The approved examples are `cooking-prep-3d.webp` for preparation, cut size, and arrangement, and `cooking-prep-photo.webp` for realistic surface texture or doneness. Live recipe references generate from the actual step, with up to six automatic images and additional images on request. Both are opaque 3:2 images and need meaningful alt text in the reference component. Their source PNGs also live in `design/assets/`. The art direction and asset audit are in [cooking illustration prompts](design/cooking-illustration-prompts.md).

While a step reference generates, reserve its 3:2 frame with a regular ink dot grid across the whole cream frame, with a slowly moving area of contrast and a quiet teal status detail. Reveal the actual image after it loads. Reduced motion keeps the dots static and removes the reveal transition. Generation and download failures keep the compact inline retry treatment. The [actual cooking fixture](design/mocks/cooking-reference.html) covers pending, ready, error, broken-image, offline, and replay states through its `mode` query parameter.
...
The cooking completion view plays one two-second canvas confetti burst on entry, then clears and hides the canvas. Room refreshes do not replay it. Confetti ignores pointer input, has no sound, and stops on navigation, viewport resize, or a reduced-motion preference change. Reduced motion skips the burst entirely. Confetti is animated decoration, never part of the ticket or bell image.
...
The starter backgrounds are opaque scenes in landscape, square, and portrait formats. The hanging sign is a 384 × 394 transparent WebP. The original `bag-pot.webp` remains available as the earlier connection illustration reference.
